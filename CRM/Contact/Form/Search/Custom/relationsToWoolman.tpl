...
cj(function() {
  cj().crmaccordions();
  cj('#woolman').change(function() {
    var val = cj(this).val();
    if (val.substring(0, 4) == 'sem_') {
      cj('.crm-contact-custom-search-form-row-semester').show();
    }
    else {
      cj('.crm-contact-custom-search-form-row-semester').hide();
    }
  }).change();
});
</script>
